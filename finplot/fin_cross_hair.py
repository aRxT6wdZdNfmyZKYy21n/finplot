...
from finplot.constants import (
    ...
    significant_eps,
    crosshair_right_margin,
    crosshair_bottom_margin,
)
...
class FinCrossHair:
    ...
    def update(self, point=None):
        ...
        if self.ax.vb.x_indexed:
            xtext,timebased = x2local_t(self.ax.vb.datasrc, x)
        else:
            xtext = round_to_significant(rng, rngmax, x, sd, se)
        linear_y = y
        y = self.ax.vb.yscale.xform(y)
        ytext = round_to_significant(rng, rngmax, y, sd, se)
        if not timebased:
            if xtext:
                xtext = 'x ' + xtext
            ytext = 'y ' + ytext
